 ...
 */
public class PapaKeyCacheBPlusTree<KEY extends Comparable<KEY> > {

	private int NODE_SIZE;
	private File indexArea;
	private Node<KEY> root;
	private boolean duplicate;
	private BPlusTreeMetaInfo papaChildKeyMap;

	public PapaKeyCacheBPlusTree(File indexRoot, String name, int nodeSize, boolean duplicate) throws IOException {
		this.duplicate = duplicate;
		NODE_SIZE = nodeSize;
		indexArea = new File(indexRoot, name);
		indexArea.mkdir();

		papaChildKeyMap = new BPlusTreeMetaInfo(indexArea, name);
	...
	private void overflowProcess(Node<KEY> s1) {
		if( s1.isOverflow() ) {
			...
			Node<KEY> s2 = splitResult.getNode(); s2.setId(papaChildKeyMap.nextIntId());
			Node<KEY> papa = null;

			if( s1.getId() != Node.ROOT_ID ) {
//				papa = Node.load(indexArea, s1.getPapaId());
				papa = Node.load(indexArea, papaChildKeyMap.getPapaId(s1.getId() ) );
			}
			...
			if( !s1.isLeaf() ) {
				for(int i = 0 ; i<s2.vector.size() ; i++ ) {
					...
					papaChildKeyMap.setPapaId(nodeId, s2.getId());
...
//				spareChild.store(indexArea);
				...
				papaChildKeyMap.setPapaId(spareNodeId, s2.getId());
			}
	...
	private void upPropagation(Node<KEY> s1, Node<KEY> s2, Node<KEY> papa, SplitResult<KEY> sr) {
		if( papa == null ) {
			//resetting id
			if(s1.getId() == Node.ROOT_ID ) {
				s1.setId(papaChildKeyMap.nextIntId());
...
//						child.store(indexArea);
						papaChildKeyMap.setPapaId(nodeId, s1.getId() );
					}
...
//					spareChild.store(indexArea);
					papaChildKeyMap.setPapaId(spareNodeId, s1.getId());
				}
			}
			...
			papaChildKeyMap.setPapaId(s1.getId(), Node.ROOT_ID);
			papaChildKeyMap.setPapaId(s2.getId(), Node.ROOT_ID);
